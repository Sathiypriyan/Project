...
class Menu //class for managing the app's menu
{
	public:
		...
		static void exitprog(); // exits the app
};
...
{				
	...
	public:
		...
		string getStop() { return this -> stop; }
};			
...
void Flight::addFlight(){	
	...
	do{
		getline(cin, temp);
		if ( (temp.length() <= 10) && (temp.compare(this -> from)) && (temp.compare(this -> to)) ) {
			this -> stop = temp;
			flag = true;
		}else {
			cout << "Please insert a valid Stop city! ";
			flag = false;
		}
	}while(!flag);
...
void Flight::displayFlightInfo(int num){
	...
	for (std::list<Flight>::iterator i = flist.begin(); i != flist.end(); ++i) {
		if (num == i -> flightNo) {
			...
			cout << left << setw(10) << "FLIGHT" << left << setw(10) << "FROM" << left << setw(10) << "TO" << left << setw(10) << "STOP" << left << setw(10) << "LEAVE" << left << setw(10) << "ARRIVE" << left << setw(10) << "COST" << left << setw(10) << "SEATS" << left << setw(10) << "BOOKED" << endl;
			cout << left << setw(10) << i -> flightNo << left << setw(10) << i -> from << left << setw(10) << i -> to << left << setw(10) << i -> stop << left << setw(10) << l_time << left << setw(10) << a_time << left << setw(10) << i -> cost << left << setw(10) << i -> seats << left << setw(10) << i -> booked_seats << endl;
...
void Flight::selectAirport(){
	...
	cout << "PLEASE ENTER THE BELOW DETAILS TO LIST THE FLIGHT SCHEDULE" << endl << endl;
	...
	do{
		getline(cin, temp);
		if ( (temp.length() <= 10) && (checkString(temp)) && (temp.compare(from)) ) {
			to = temp;
			flag = true;
		}else{
			cout << "Please insert a valid Destination city! ";
	...
	for (std::list<Flight>::iterator i = flist.begin(); i != flist.end(); ++i) {
		...
		if ( ( (i->from==from) || (i->stop==from) )
			&& ( (i->to==to) || (i->stop==to) ) ) {
			
			if (flag) {
				cout << "\n\t\t\t\t AVAILABLE FLIGHTS" << endl << endl;
				cout << left << setw(10) << "FLIGHT" << left << setw(10) << "FROM" << left << setw(10) << "TO" << left << setw(10) << "STOP" << left << setw(10) << "LEAVE" << left << setw(10) << "ARRIVE" << left << setw(10) << "COST" <<left << setw(10) << "SEATS" << left << setw(10) << "BOOKED" << endl;
			}
	...
	if (!flist.empty()) {
		...
		if (choice == "y" || choice == "Y") {			
			...
			do{				
				...
				if ( (this -> nationality.length() <= 10) && (checkString(this -> nationality)) ){
					flag = true;
				}else {
...
bool Person::displayPersonInfo(int passport){
	...
	for (std::list<Person>::iterator i = plist.begin(); i != plist.end(); ++i){
		...
		if(passport == i -> passportNo){
			...
			cout << left << setw(15) << "NAME" << left << setw(15) << "SURNAME" << left << setw(15) << "PASSPORT_No" << left << setw(15) << "NATIONALITY" << left << setw(15) << "ADDRESS" << left << setw(15) << "TELEPHONE" << endl;
			
			cout << left << setw(15) << i -> name << left << setw(15) << i -> surname << left << setw(15) << i -> passportNo << left << setw(15) << i -> nationality << left << setw(15) << i -> address << left << setw(15) << i -> tel << endl << endl;
			cout << "Your flight(s): ";
